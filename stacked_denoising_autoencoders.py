...
class StackedDenoisingAutoencoders(object):
    ...
    def __init__(self, n_in: int, hidden_layer_sizes, n_out: int, rng: np.random.RandomState):
        ...
        for i in range(self.n_layers):
            hidden_layer_n_in = n_in if i == 0 else hidden_layer_sizes[i-1] # n_out of the (i-1)-th hidden layer
            ...
            self.sigmoid_layers.append(
                HiddenLayer(hidden_layer_n_in, n_out=hidden_layer_sizes[i],
                            W=None, b=None, rng=rng, activation="sigmoid"))

            # construct DA layers
            self.da_layers.append(
                DenoisingAutoencoders(n_visible=hidden_layer_n_in,
                                      n_hidden=hidden_layer_sizes[i], # n_out of the i-th hidden layer
                                      W=self.sigmoid_layers[i].W, # shared with the sigmoid layer
                                      h_bias=self.sigmoid_layers[i].b, # shared with the sigmoid layer
                                      v_bias=None, rng=rng))
    ...
    # not use labels T
    def pretrain(self, X, minibatch_size: int, minibatch_N: int, epochs: int, learning_rate: float, corruption_level: float):
                ...
                for batch in range(minibatch_N):
                    ...
                    if layer == 0:
                        ...
                        for i in range(minibatch_size):
                            X_[i] = self.sigmoid_layers[layer-1].output(prev_layer_X_[i])

                    self.da_layers[layer].train(X_, minibatch_size, learning_rate, corruption_level)
