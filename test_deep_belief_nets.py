...
from deep_belief_nets import DeepBeliefNets
from dataset import make_binomial_dataset, make_binomial_labels

def test_deep_belief_nets():

    print("set configs")

    rng = np.random.RandomState(123)

    train_N_each = 50 # 200: slow
    validation_N_each = 50 # 200: slow
    test_N_each = 100 # 50
    ...
    classifer = DeepBeliefNets(n_in, hidden_layer_sizes, n_out, rng)
    print("done.")

    print("pre-training the model...")
    classifer.pretrain(train_X_minibatch, minibatch_size, train_minibatch_N,
                       pretrain_epochs, pretrain_learning_rate, k)
    print("done.")
